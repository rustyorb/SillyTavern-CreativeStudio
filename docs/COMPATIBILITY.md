...
| Expression sprites upload `/api/sprites/upload` (form: `name`, `label`, `spriteName`, `avatar` file) | Source `src/endpoints/sprites.js:239-290`; Runtime (8 sprites to `characters/Tinker/`) | 1.19.0 | Folder = the character's name, or the Character Expressions override for its avatar file (`extension_settings.expressionOverrides`, source `expressions/index.js:625-637`) |
...
| Pictures inside a ComfyUI workflow | Source `public/scripts/extensions/stable-diffusion/index.js:4251-4271` (ST's own `%char_avatar%` placeholder becomes base64 text, which ComfyUI can read only with a base64 loader node); Runtime (`easy loadImageBase64` of ComfyUI-Easy-Use) | 1.19.0 | The studio sends the picture inline through `easy loadImageBase64` or `ETN_LoadImageBase64`; otherwise it uploads to ComfyUI's `/upload/image` from the browser (SillyTavern has no upload route; without ComfyUI's CORS header the answer cannot be read, so the file name is fixed up front) |
| Refused graph vs failed run | Source `src/endpoints/stable-diffusion.js:562-632` (`/generate`: a `/prompt` refusal answers "ComfyUI returned an error."; a failed run answers "ComfyUI generation did not succeed" with node errors; the first image output is returned) | 1.19.0 | The studio tries alternatives only while nothing has worked yet; every studio graph has exactly one image output |
| Diffusion-only models in the model list | Source `src/endpoints/stable-diffusion.js:421-446` (`UNet: …`/`GGUF: …` entries); Runtime (Nunchaku FLUX Kontext found) | 1.19.0 | A FLUX Kontext model found here is used for sprites made from a character's picture |
| Expression sprites list `/api/sprites/get?name=<folder>` | Source `src/endpoints/sprites.js:118-150` (label = file name up to the first `-` or `.`; several files may share a label); Runtime (Seraphina: 28 labels) | 1.19.0 | The studio takes one picture per label, preferring the plain `<label>.png` |
| Character ⇄ lorebook link | Source `world-info.js` (character lore = `data.extensions.world`, a World Info file name; `src/endpoints/worldinfo.js:151` files books under `sanitize(name).json`); Runtime (created character stored the link) | 1.19.0 | The studio sets the link on apply/create when the card has none, using ST's file name for the book; extra per-character books (`charLore`) are not set |
