...
- ✅ *Just build it for me* (Characters), *Build the rest for me* (only what the character lacks; never duplicates artifacts), *Critique & fix*
- ✅ **One AI box on every text field** (card fields and lorebook entry content): a typed direction, then *Add to it* (the model returns only new material plus how it joins; the text already there is kept word for word), *Rewrite*, *3 takes*, or one-click *More detail / Tighter / Punchier*; *Write it* when the field is empty. Live on Seraphina (OpenRouter → DeepSeek): *Add to it* on her 2,844-character description kept it exactly and added a `[Seraphina's fears= …]` block in the card's own W++ format (3.0 s); on the Eldoria entry it kept the dialogue and continued it in the same `{{user}}:` / `{{char}}:` form (3.2 s)
- ✅ **AI instructions** page (content pill in the top bar, the plug panel, or the palette): every instruction the studio gives the model, grouped, searchable, editable, with *Reset* and *Reset all*; edits live in SillyTavern's extension settings (and the studio's settings backup) and apply to every project
- ✅ **Content level per project**: *Keep it SFW*, *Mature themes* (default) or *Adult, explicit allowed*; its rule goes into every writing task (not into regex/STscript/preset engineering), mature and adult say "fiction by and for adults, no disclaimers, no toning down" and always add the fixed line "Anyone in romantic or sexual content is an adult."; the critique judges craft, not morality; the generator's rating dial sets it too
...
- ✅ **Sprites from a character's own picture** (imported avatars above all; setting under Images): a **FLUX Kontext** edit when the server has a Kontext model (found in SillyTavern's model list as a UNet/GGUF entry; Nunchaku `svdq-`, GGUF or plain loader; T5 and Flux VAE found by name), otherwise the **face alone repainted** (Impact Pack FaceDetailer + Ultralytics face model), otherwise a gentle **whole-picture** repaint. The picture goes in inline (Easy-Use `easy loadImageBase64` or tooling-nodes `ETN_LoadImageBase64`) or by a direct upload; cutouts by **InSPyReNet** (keeps long hair that isnet loses), then isnet. The ways a server accepts are found by trying (a refused graph costs nothing) and remembered per server. Darker moods tell the editor what replaces the smile. Live on ST's Seraphina (anime avatar): Kontext kept the art style, outfit and pose exactly; about 35 s a sprite at 1 MP / 20 steps (0.75 MP / 16 steps was faster but lost subtle moods such as sadness). Informed by the author's own ComfyUI expression workflows and the community "Silly Tavern V2" FaceDetailer workflow
- ✅ **Pictures from a character's own picture**: with Kontext on the server, *Paint* makes the portrait, full body and scene from the avatar (the scene on a wide canvas, "with no people in it"); without it, pictures are painted from the words as before. Live: Seraphina's three AI-written prompts painted from her avatar in 130 s, same face, outfit and art style
- ✅ Edit wording tested on Kontext: "Remove the smile: … lips closed in a straight line" is what moves a smiling face to neutral; concrete cues (star-like highlights, both eyes closed, a small round open mouth) separate the quiet positive moods
...
- ✅ Pictures follow the content level: SFW adds safe tags and keeps nudity out; mature keeps only nudity out; adult keeps nothing out
- 🟡 Freshly painted sprites share one face with each other; sprites made from the avatar match it. Face-only and whole-picture repaints follow the checkpoint's style, so an anime avatar wants an anime checkpoint (Kontext has no such limit)
- 🟡 Kontext renders subtle moods (sadness, remorse) more gently than strong ones (fear, anger, joy)
...
- ✅ **Bring in from SillyTavern** with what belongs to the character: card + avatar, its linked lorebook (`data.extensions.world`) and its expression sprites (`/api/sprites/get`, Character Expressions folder override respected). Entry points: a feather in ST's character panel (next to Export; returns to the project that already holds the character, otherwise a new project named after it), *Start from one of my characters…* on the Project page, Characters → *From SillyTavern…*, and the command palette. The picker lists the current chat's character first, then by most recent chat, and marks characters already in the project. Live: ST's Seraphina with lorebook *Eldoria* and 28 sprites in 2.6 s; applying the untouched copy back reported no differences
- ✅ Review diff and apply (merge-attributes) with backup/restore, create in ST with fidelity read-back. The card SillyTavern receives names the character's first project lorebook as its World Info link (by ST's file name), so generated lorebooks are linked in ST without manual wiring (live: created character stored `extensions.world`)
- ✅ *Build the rest for me* says what it will add for the character in view (openings, lorebook, preset, regex, Quick Replies, image prompts, portrait)
